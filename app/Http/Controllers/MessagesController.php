...
class MessagesController extends Controller
{
    ...
    public function edit_filled_message($id)
    {
        $message = DB::table('messages')->where('id',$id)->first();
        $aftnraw = DB::table('aftn_header')->where('message_id',$id)->first();
        $add = DB::table('additional_informations')->where('message_id',$id)->first();

        $aftn = get_object_vars($aftnraw);
        return view('edit-fpl',compact('message','aftn','add'));
    }

    public function edit_filled_messagePost(Request $request,$id)
    {
        $idmessage = $id; 
        $iduser = Auth::user()->id;
        $user = DB::table('users')->where('id',$iduser)->first();
        ...
        DB::table('messages')->where('id',$idmessage)->update([
            'user_id' => $user->id,
            'type' => $type,
            'aircraft_id' => $validateData['aircraft-id'],
            'dep_id' => $validateData['dep-id'],
        ...
        $ATK = DB::table('aftn_header')->where('message_id',$idmessage)->update([
            'priority' => $validateData['priority'],
            'user_id' => $iduser,
            'originator' => $user->name,
            'filing_time' => $validateData['filing-time'],
        ...
        $survival = implode(" ",$validateData['supp_survival']);
        $jacket = implode(" ",$validateData['supp_jacket']);
        DB::table('additional_informations')->where('message_id',$idmessage)->update([
            'route' => $validateData['routeP'],
            'STS' => $validateData['STS'],
            'PBN' => $validateData['PBN'],
            'NAV' => $validateData['NAV'],
        ...
        return redirect()->route('incomingMessage')->with('berhasil','Pesan telah berhasil diupdate');
    }
    ...
    public function arrivalPost(Request $request)
    {
        ...
        $validateData = $request->validate([
            ...
            'arrival-aerodrome' => 'string|nullable',
            'filled-by' => 'required|string',
            'address1' => 'nullable|string',
            'address2' => 'nullable|string',
            'address3' => 'nullable|string',
